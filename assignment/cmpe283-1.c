...
/*
 * Pinbased procbased capabilities
 * See SDM volume 3, table 24-6
 */
...
/*
 * Primary Exit controls
 * See SDM volume 3C, table 24-13
 */
...
/*
 * Primary Entry controls
 * See SDM volume 3C, table 24.15
 */
...
detect_vmx_features(void)
{
	...
	pr_info("Secondary Process based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secondary_procbased, 25, lo, hi);
	...
	pr_info("VM Exit based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_controls, 16, lo, hi);
}
